...
LogLoss is an error calcualtion used to determine how close the predictions of a model are to the actual values. 
...
Unlike AUC, logloss is robust in the prescence of imbalanced classes. It takes into account the certainty of the prediciton.  
...
For example, let's say you have a dataset qith 10,000 rows and only 100 are the target.  Then you could technically classify the whole set as "not the target" and get an AUC of 99% - which while technically correct, isn't very useful. 

There are other ways to get around this like percision, recall and ther harmonic mean (F1 score), but logloss is a more direct measure of what you are attempting ot achieve - accurate probabilities.
...
Below you can see a representation of predicted and actual probabilities.  
  ...
  The top example depicts a poor prediction, where there is a large difference between the predicted and acutal, this is results in a large LogLoss.  This is good because the function is penalizing a wrong answer that the model is "confident" about. 
...
I decided to look into it by creating two models (one good, one bad) on a famous datascience dataset called the [Titanic dataset](https://www.kaggle.com/c/titanic/data).
...
You can see above that the standard devation (or spread) of the LogLoss is greater than the mean in the "Good" model and almost as large as the mean in the "Poor" model.  This, combined with the shape of the distributions tells me that the mean may not be the best representation of LogLoss. 

If you look at the last columm, **Median**, these values appear to represent the distributions better. 
...
**It turns out yes, yes you can do that.  The "Good" model had a better LogLoss for both the minority and majority calss. This is a viable way to compare model performance in classification problems, and should be used over techniques like AUC. **
...
4. You can break  LogLoss down even further to look at the minority and majority class. 
